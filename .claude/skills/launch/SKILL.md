...
3. **Launch in the background.** Start `npm run coach` from `tools/` as a background process (it's a long-running server — never run it in the foreground, it would block the session). Give it a moment, then confirm the port is answering.
4. **Report plainly:**
